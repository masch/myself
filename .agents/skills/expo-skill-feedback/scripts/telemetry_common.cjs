...
const POSTHOG_PROJECT_API_KEY =
  process.env.EXPO_SKILLS_POSTHOG_KEY ||
  "phc_w8xRytdAAwkV3oExnuUozqH64PMzCmDLnyoChpPBcNXs";
...
function telemetryEnvSignal() {
  ...
  const flag = String(process.env.EXPO_SKILLS_TELEMETRY || "")
    .trim()
    .toLowerCase();
...
function telemetryActive() {
  ...
  try {
    if (fs.existsSync(OPT_IN_PATH)) return true;
  } catch {}
  return DEFAULT_ON;
}
...
function readInstallationId(create = true) {
  try {
    ...
    fs.mkdirSync(path.dirname(INSTALLATION_ID_PATH), {
      recursive: true,
      mode: 0o700,
    });
    try {
      fs.chmodSync(path.dirname(INSTALLATION_ID_PATH), 0o700);
    } catch {}
    ...
    try {
      ...
      try {
        fs.writeFileSync(fd, installationId + "\n");
      } finally {
        fs.closeSync(fd);
      }
      ...
      if (err && err.code === "EEXIST")
        return fs.readFileSync(INSTALLATION_ID_PATH, "utf8").trim() || null;
      throw err;
    }
  } catch {
    return null;
...
function telemetryIdentity(agentHarness, { createInstallation = true } = {}) {
  ...
  const installHash = id
    ? crypto.createHash("sha256").update(id).digest("hex").slice(0, 32)
    : null;
  if (installHash)
    return [
      `expo-skills-installation:${installHash}`,
      { installation_id_hash: installHash },
    ];
    ...
    const req = https.request(
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          reject(
            new Error(
              `HTTP ${status} ${Buffer.concat(chunks).toString("utf8")}`,
            ),
          );
        });
      },
