...
Every TTI event carries automatic params in three groups: frame rate, device state, and network state. Read the frame-rate group to classify _what kind_ of slowness you're seeing, then read the device and network groups to decide whether the cause is the code or the conditions.
