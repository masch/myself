...
**The picker flags and the session ID are mutually exclusive.** `--event-name`, `--sort`, `--days`, `--start`, and `--end` describe how to _find_ a session, so passing any of them together with a session ID throws. Query a known session with the ID alone.
